...
function collectFilters() {
  ...
  if (priceRange) filters.max_price = parseFloat(priceRange.value);
...
async function fetchAndRender() {
  ...
  const res = await fetch(API_URL, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({type: 'GetPackages', ...filters})
  });
      ...
      <div class="pkg-body">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-top:0.375rem;">
          ...
          <button class="btn btn-primary btn-sm"
                  onclick="event.preventDefault(); bookPackage(${pkg.Package_ID})">Book</button>
        </div>
      </div>
    </a>
  `;
...
function updateActiveFilterTags(filters) {
  ...
  if (filters.min_rating) tags.push(`${filters.min_rating} & up`);
...
function bookPackage(packageId) {
  const user = getUser();
  if (!user) {
    window.location.href = `login.html?redirect=package-detail.html?id=${packageId}`;
    return;
  }
